...
function loadLife200(x){
    game.addCollectable(new collectable('life',false, new game.canvas.vector(x,200),30,30,
                    {ethereal:true, movable:true, name:'life111',
                     offset:{x:0,y:0},
                     sprite: {
                                static: true,
                                img: library.loadImages.life,
                                left: {
                                    img: library.loadImages.life,
                                    spritesheet: [100, 100],
                                    idle:[28, 1, 15],
                        ...
                        effects: { destroyAfter:true, callback: function(){
                                           ...
                                           //     var col1 = game.getCollectableByName('life111')
                                           //      col1.self.sprite.right.idle.reset();
                                                //this.sprite.right.idle.reset();
                                            }
                                 }      
                    })
               );
    ...
    cinematicsData = {data:{}, 
                      action:function(){
                          ...
                          var positionPlayerX = game.players[0].position.x;
                            ...
                            {
                                case 1:
                                   loadLife200(positionPlayerX+50); 
                                   addEnemy(10,1, library.loadImages.enemy1, 20, 20, function(){ loadCoin(positionPlayerX)});
                                   ...
                                   break;
                                case 2:
                                   addEnemy(10,1, library.loadImages.enemy1, 20, 20, function(){ loadCoin(positionPlayerX); loadLife200(positionPlayerX) });
                                   addEnemy(game.canvas.container.width+10,1, library.loadImages.skeleton, 900, 20, function(){
                                       loadCoin(positionPlayerX);
                                        addEnemy(20,1, library.loadImages.enemy2, 800, 20, function(){
                                            loadCoin(positionPlayerX);
                                            addEnemy(game.canvas.container.width+100,1, library.loadImages.skeleton, 900, 20, function(){ loadCoin(positionPlayerX)});
                                        });
                                   });
                                  break;
                                case 3:
