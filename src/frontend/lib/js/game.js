var game = {
   ...
   enableInput: function(){
    ...
    render: function(){
        ...
        if(!game.pauseInput && game.players[0].position.x > game.canvas.container.width*9/10 && game.creatures.length == 0){
    ...
    getCollectableByName: function(name){
        for (var i = 0; i < game.collectables.length; i++) {
            if(game.collectables[i].name==name){
                return game.collectables[i];
            }
       }
       return false;
    },    
